  ...
  getDados().forEach((atividade) => {
    if (atividade.dataInicio === dayString && getDados() != []) {
      ...
      const getLocalStorage = () =>
        JSON.parse(localStorage.getItem("db_categoria")) ?? [];
      getLocalStorage().forEach((categoria) => {
        if (atividade.categoria == categoria.nome) {
          if (categoria.cor == "Azul") eventDiv.classList.add("blueText");
          ...
          else if (categoria.cor == "Rosa") eventDiv.classList.add("pinkText");
        }
      });
      ...
      if (
        ...
        parseInt(minutos) >= parseInt(minutosIntervalo)
      ) {
